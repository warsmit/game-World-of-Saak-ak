...
, hasWin(false)
{
    ...
    mFont.loadFromFile("STENCIL.TTF");
    mText.setFont(mFont);
...
void World::update(float time)
{
    ...
    if(!hasWin)
    {
        ...
        /*if(!hasWin)
        {
            printf("%i %i %i %i\n", playersData[0].result,playersData[1].result,playersData[2].result,playersData[3].result);
        }*/
        ...
        //Коллозия между игроками, если не сохранит ьв tX и tY , то 1 всегда будет другйо толкать а другой нет!
                ...
                if(i->isAlive())
                    if (i->getBody().intersects(j->getBody()))
                    {
                        ...
                        if(dotProduct > 0)
                        {
                            ...
                            newVelX2 -= collisionWeightB * xCollision;//1
                            newVelY2 -= collisionWeightB * yCollision;

                            first.setDSpeed(newVelX1, newVelY1);
                            //second.setDSpeed(newVelX2, newVelY2);
                        }
